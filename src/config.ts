...
export const Config = Schema.intersect([
  ...
  }).description('🚪 玩家加入消息转发配置（进服通知）【服务器 → 聊天平台】<br>⚠️ 会尊重 targetPlatformChannelList 配置项，只发送到已启用的目标频道'),
  ...
  }).description('🚶 玩家离开消息转发配置（离服通知）【服务器 → 聊天平台】<br>⚠️ 会尊重 targetPlatformChannelList 配置项，只发送到已启用的目标频道'),
